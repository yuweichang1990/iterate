...
Build 模式**不是**「先寫好一份完整計畫，然後照表操課」。雖然第一輪會寫出 `00-plan.md` 作為初始方向，但**每一輪結束時，Claude 會根據當下觀察到的情況動態決定下一步該做什麼**（透過 `<explore-next>` 標籤）。這意味著：

- 實作過程中發現新問題 → 下一輪會優先處理它，而不是繼續原本計畫
- 某個子任務比預期簡單 → 可以在同一輪順便做下一件事
- 發現原本的架構設計有問題 → 下一輪會調整方向

這讓 Build 模式具備**自我觀察和自我修正**的能力，而不只是機械式執行。
...
stateDiagram-v2
    ...
    state 階段一 {
        [*] --> 初始規劃: 第 1 輪：寫 00-plan.md
        初始規劃 --> 觀察與實作: 以初始計畫為方向
        觀察與實作 --> 動態決策: 這輪做完了
        動態決策 --> 觀察與實作: explore-next：\n根據當前狀態決定下一步\n（可能偏離原始計畫）
        動態決策 --> [*]: 核心任務完成
    }
    ...
    state 階段二 {
        [*] --> 回顧: 重新審視整個成果
        回顧 --> 發現問題: 有值得改進的地方
        發現問題 --> 改進: 修 Bug / 強化 / 補測試
        改進 --> 回顧: explore-next enhance:\n再次觀察有沒有遺漏
        回顧 --> [*]: 沒有值得改進的了
    }
...
**三個階段的核心差異不是「做什麼」，而是「用什麼視角觀察」**：

| 階段 | 觀察視角 | 問自己的問題 | 比喻 |
|------|---------|-------------|------|
| **階段一** | 開發者 | 「功能完成了嗎？能跑嗎？」 | 蓋房子 |
| **階段二** | 資深工程師 | 「這段程式碼能通過 code review 嗎？」 | 驗收 |
| **階段三** | 產品經理 | 「第一次用的人會不會卡住？」 | 裝潢 |

每個階段都遵循同一個循環：**觀察 → 發現問題 → 修正 → 再觀察**。差別只在於「用什麼標準來觀察」。
